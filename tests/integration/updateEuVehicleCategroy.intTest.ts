    ...
    it("should return 200 and the updated vehicle if it has a euVehicleCategory as undefined", async () => {
      ...
      await LambdaTester(updateEuVehicleCategory)
        ...
        .expectResolve((result: any) => {
          ...
          expect(techRecordWrapper.techRecord[0].euVehicleCategory).toBe(
            undefined
          );
          expect(techRecordWrapper.techRecord[0].statusCode).toEqual(
            STATUS.ARCHIVED
          );
          expect(techRecordWrapper.techRecord[0].lastUpdatedByName).toEqual(
            createdByName
          );
          expect(techRecordWrapper.techRecord[0].lastUpdatedById).toEqual(
            createdById
          );
          expect(techRecordWrapper.techRecord[0].updateType).toEqual(
            UPDATE_TYPE.TECH_RECORD_UPDATE
          );
          expect(techRecordWrapper.techRecord[1].euVehicleCategory).toBe(
            EU_VEHICLE_CATEGORY.M1
          );
          expect(techRecordWrapper.techRecord[1].statusCode).toEqual(
            STATUS.CURRENT
          );
          expect(techRecordWrapper.techRecord[1].createdById).toEqual(
            createdById
          );
          expect(techRecordWrapper.techRecord[1].createdByName).toEqual(
            createdByName
          );
        });
    });
    ...
    it("should return 400 if the euVehicleCategory does not exist", async () => {
      ...
      await LambdaTester(updateEuVehicleCategory)
        ...
        .expectResolve((result: any) => {
          ...
          expect(JSON.parse(result.body)).toBe(
            HTTPRESPONSE.INVALID_EU_VEHICLE_CATEGORY
          );
        });
    });
