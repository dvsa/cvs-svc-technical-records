...
describe("getTechRecordsList", () => {
  ...
  context("when db call returns data", () => {
    it("should return a populated response", async () => {
      ...
      const returnedRecords: any = await techRecordsService.getTechRecordsList(
        "1B7GG36N12S678410",
        STATUS.CURRENT,
        SEARCHCRITERIA.ALL
      );
  ...
  context(
    "and the statusCode by which we query is provisional_over_current",
    () => {
      context(
        "and the result is a techRecord with one provisional entry and one current one",
        () => {
          it("should return an array containing the provisional entry", async () => {
            ...
            const mockDAO = new MockDAO();
            const techRecordsService = new TechRecordsService(mockDAO);
            const techRecordsListHandler =
              new TechRecordsListHandler<PublicServiceVehicle>(mockDAO);

            let expectedResult: PublicServiceVehicle = JSON.parse(
              JSON.stringify(records[9])
            );

            expectedResult.techRecord.splice(0, 1);
            expectedResult =
              techRecordsListHandler.formatTechRecordItemForResponse(
                expectedResult
              );
            const returnedRecords = await techRecordsService.getTechRecordsList(
              "YV31MEC18GA011911",
      ...
      context(
        "and the result is a techRecord with one archived entry and one current one",
        () => {
  ...
  context("when db returns empty data", () => {
    it("should return 404-No resources match the search criteria", async () => {
      ...
      try {
        expect(
          await techRecordsService.getTechRecordsList(
            "Rhubarb",
            "Potato",
            SEARCHCRITERIA.ALL
          )
  ...
  context("when db returns more than 1 result", () => {
    it("should return all in an array", async () => {
      ...
      const techRecordsService = new TechRecordsService(mockDAO);
      expect.assertions(1);
      const retVal = await techRecordsService.getTechRecordsList(
        "",
        "all",
        SEARCHCRITERIA.ALL
      );
  ...
  context("when searching for a vehicle with euroStandard field set", () => {
    ...
    it("should return euroStandard as a string, even if the field is set as 0 in dynamodb", async () => {
      ...
      const techRecordsService = new TechRecordsService(
        new MockDAO(techRecordWithNumber)
      );
      ...
      expect(typeof returnedRecords[0].techRecord[0].euroStandard).toBe(
        "string"
      );
    ...
    it("should return euroStandard as a string when the field is already a string", async () => {
      ...
      const techRecordsService = new TechRecordsService(
        new MockDAO(techRecordWithString)
      );
    ...
    it("should return euroStandard as null if it has been set as null", async () => {
      ...
      const techRecordsService = new TechRecordsService(
        new MockDAO(techRecordWithNull)
      );
...
describe("insertTechRecord", () => {
    ...
    it("should return 201 Technical Record Created", async () => {
      ...
      const MockDAO = jest.fn().mockImplementation(() => {
        return {
          ...
          getSystemNumber: () =>
            Promise.resolve({ systemNumber: "10000001", testNumberKey: 3 }),
          getTrailerId: () => {
            return Promise.resolve({
              trailerId: "C530001",
              trailerLetter: "C",
      ...
      const data: any = await techRecordsService.insertTechRecord(
        techRecord,
        msUserDetails
      );
  ...
  context(
    "when trying to create a new technical record with invalid payload",
    () => {
      it("should return validation error 400", async () => {
        const MockDAO = jest.fn().mockImplementation(() => {
          return {
            ...
            getSystemNumber: () =>
              Promise.resolve({ systemNumber: "10000001", testNumberKey: 3 }),
            getTrailerId: () =>
              Promise.resolve({ trailerId: "C320001", testNumberKey: 2 }),
          };
        });
        const techRecordsService = new TechRecordsService(new MockDAO());
        ...
        try {
          expect(
            await techRecordsService.insertTechRecord(techRecord, msUserDetails)
          ).toThrowError();
        } catch (errorResponse) {
      ...
      context("and the primaryVrm and secondaryVrms are not valid", () => {
        it("should return Primary or secondaryVrms are not valid error 400", async () => {
          ...
          try {
            expect(
              await techRecordsService.insertTechRecord(
                techRecord,
                msUserDetails
              )
            ).toThrowError();
  ...
  context(
    "when trying to create a technical record for existing vehicle",
    () => {
  ...
  context("when trying to create a new trailer", () => {
    ...
    context("and the trailer id generation failed", () => {
      context(
        "and the trailer id object doesn't contain the trailerId attribute",
        () => {
          it("should return error 500 TrailerId generation failed", async () => {
            ...
            try {
              ...
              expect(errorResponse.body).toEqual(
                ERRORS.TRAILER_ID_GENERATION_FAILED
              );
            }
          });
        }
      ...
      context("and the trailer id object contains the error attribute", () => {
        it("should return error 500 TrailerId generation failed", async () => {
          ...
          try {
            ...
            expect(errorResponse.statusCode).toEqual(500);
            expect(errorResponse.body).toEqual(
              "Some error from test-number microservice"
            );
          }
        });
      });
      ...
      context("and the test-number microservice returned an error", () => {
        it("should return error 500", async () => {
          ...
          try {
            ...
            expect(errorResponse.statusCode).toEqual(500);
            expect(errorResponse.body).toEqual(
              "Error from test-number microservice"
            );
          }
        });
      });
  ...
  context("when trying to create a new vehicle", () => {
    context("and the system number generation is successfull", () => {
      it("should set the correct system number on the vehicle", async () => {
        ...
        expect(await numberGenerator.generateSystemNumber()).toEqual(
          "10001111"
        );
      });
    });
    ...
    context("and the system number generation failed", () => {
      context(
        "and the system number object doesn't contain the systemNumber attribute",
        () => {
          it("should return error 500 System Number generation failed", async () => {
            ...
            try {
              expect(
                await numberGenerator.generateSystemNumber()
              ).toThrowError();
            } catch (errorResponse) {
              expect(errorResponse.statusCode).toEqual(500);
              expect(errorResponse.body).toEqual(
                ERRORS.SYSTEM_NUMBER_GENERATION_FAILED
              );
            }
          });
        }
      ...
      context(
        "and the system number object contains the error attribute",
        () => {
          it("should return error 500 System Number generation failed", async () => {
            const MockDAO = jest.fn().mockImplementation(() => {
              return {
...
describe("updateTechRecord", () => {
  ...
  context("when updating a technical record for an existing vehicle", () => {
    it("should return the updated document", async () => {
      ...
      techRecord.techRecord[0].adrDetails.vehicleDetails.type =
        "Centre axle tank";
      const vrms = [
        { vrm: "BBBB333", isPrimary: true },
        { vrm: "CCCC444", isPrimary: false },
      ];
      const MockDAO = jest.fn().mockImplementation(() => {
        return {
          updateSingle: () => {
      ...
      const techRecordsService = new TechRecordsService(new MockDAO());
      const updatedTechRec: any = await techRecordsService.updateTechRecord(
        techRecord,
        msUserDetails
      );
      ...
      expect(updatedTechRec.techRecord[0].bodyType.description).toEqual(
        "skeletal"
      );
    ...
    it("should get the provisional document to update", async () => {
      ...
      const vrms = [
        { vrm: "LKJH654", isPrimary: true },
        { vrm: "POI9876", isPrimary: false },
      ];
      const MockDAO = jest.fn().mockImplementation(() => {
        return {
          updateSingle: () => {
    ...
    context("and the payload doesn't pass the validation", () => {
      it("should return error 400 Payload is not valid", async () => {
        ...
        try {
          expect(
            await techRecordsService.updateTechRecord(
              recordToUpdate,
              msUserDetails
            )
          ).toThrowError();
    ...
    context("and the user wants to update an archived record", () => {
      it("should return error 400 Cannot update an archived record", async () => {
        ...
        try {
          expect(
            await techRecordsService.updateTechRecord(techRecord, msUserDetails)
          ).toThrowError();
        } catch (errorResponse) {
          expect(errorResponse).toBeInstanceOf(HTTPError);
          expect(errorResponse.statusCode).toEqual(400);
          expect(errorResponse.body.errors).toContain(
            ERRORS.CANNOT_USE_UPDATE_TO_ARCHIVE
          );
        }
      });
    });
    ...
    context(
      "and the user wants to use the update API to archive a record",
      () => {
    ...
    context(
      "and the user wants to change a record from current to provisional",
      () => {
        it("should return error 400 Cannot change current status to provisional", async () => {
          ...
          try {
            ...
            expect(errorResponse.statusCode).toEqual(400);
            expect(errorResponse.body.errors).toContain(
              ERRORS.CANNOT_CHANGE_CURRENT_TO_PROVISIONAL
            );
          }
        });
      }
  ...
  context(
    "when trying to update a technical record for non existing vehicle",
    () => {
...
describe("updateEuVehicleCategory", () => {
  ...
  context(
    "when updating a euVehicleCategory for an existing vehicle where the value is null",
    () => {
      it("should update the euVehicleCategory with the value provided", async () => {
        ...
        expectedTechRecord.techRecord[0].euVehicleCategory =
          newEuVehicleCategory;
        const MockDAO = jest.fn().mockImplementation(() => {
          return {
            updateSingle: () => {
              return Promise.resolve({
        ...
        const mockDAO = new MockDAO();
        const techRecordsService = new TechRecordsService(mockDAO);
        const response: HTTPResponse | HTTPError =
          await techRecordsService.updateEuVehicleCategory(
            systemNumber,
        ...
        expect(updatedTechRec.techRecord[0].euVehicleCategory).toEqual(
          newEuVehicleCategory
        );
      });
    }
  ...
  context(
    "when updating the euVehicleCategory for a vehicle whose VIN has changed",
    () => {
      it("should update the euVehicleCategory with the value provided", async () => {
        const expectedTechRecord = cloneDeep(
          records[171]
        ) as ITechRecordWrapper;
        ...
        const MockDAO = jest.fn().mockImplementation(() => {
          return {
            ...
            getBySearchTerm: () => {
              return Promise.resolve(
                cloneDeep([records[169], records[170], records[171]])
              );
            },
          };
        });
  ...
  context(
    "when updating a euVehicleCategory for an existing vehicle where the value is not null",
    () => {
      it("should throw error No EU vehicle category update required", async () => {
        ...
        expect(response.body).toEqual(
          `"${HTTPRESPONSE.NO_EU_VEHICLE_CATEGORY_UPDATE_REQUIRED}"`
        );
      });
    }
  );
  ...
  context("when finding more than one non-archived tech-records", () => {
    it("should throw error More than one non-archived records found", async () => {
      ...
      try {
        ...
        expect(error.body.errors).toContain(
          HTTPRESPONSE.EU_VEHICLE_CATEGORY_MORE_THAN_ONE_TECH_RECORD
        );
      }
    });
  });
  context("when finding only archived tech-records", () => {
    it("should throw error Cannot update archived tech-record", async () => {
      ...
      try {
        ...
        // FIXME: from array to string
        expect(error.body.errors).toContain(
          ERRORS.CANNOT_UPDATE_ARCHIVED_RECORD
        );
      }
    });
  });
...
describe("archiveTechRecordStatus", () => {
  ...
  context(
    "A technical record status is updated to archived via PUT verb",
    () => {
      it("should change the records status to archived and set audit details and create no other record", async () => {
        ...
        expect.assertions(2);
        const techRecordsService = new TechRecordsService(new MockDAO());
        const updatedTechRec: any =
          await techRecordsService.archiveTechRecordStatus(
            techRecord.systemNumber,
            techRecord,
            msUserDetails,
            reasonForArchiving
          );
        expect(updatedTechRec.techRecord[0].statusCode).toEqual(
          STATUS.ARCHIVED
        );
  ...
  context(
    "when trying to archive a record which was sent with changed attributes on it",
    () => {
      it("should return Error 400 Cannot archive tech record with attribute changes", async () => {
        ...
        try {
          ...
          expect(errorResponse.statusCode).toEqual(400);
          expect(errorResponse.body.errors).toContain(
            ERRORS.CANNOT_ARCHIVE_CHANGED_RECORD
          );
        }
      });
    }
      ...
      it("should change the records status to archived and set audit details and create no other record", async () => {
        ...
        techRecordsService
          ...
          .catch((err) => {
            ...
            expect(err.body.errors).toContain(
              ERRORS.CANNOT_UPDATE_ARCHIVED_RECORD
            );
          });
      });
  ...
  context("when updating the statusCode for an existing vehicle", () => {
    context("and the vehicle has only a current tech record", () => {
      it("should create new CURRENT, set the audit details and archive the previous current record", async () => {
        ...
        const mockDAO = new MockDAO();
        const techRecordStatusHandler =
          new TechRecordStatusHandler<HeavyGoodsVehicle>(
            new TechRecordsListHandler<HeavyGoodsVehicle>(mockDAO)
          );
        const updatedTechRec =
          await techRecordStatusHandler.prepareTechRecordForStatusUpdate(
            systemNumber,
            undefined,
            createdById,
            createdByName
        ...
        expect(updatedTechRec.techRecord[1].createdByName).toEqual(
          createdByName
        );
        expect(updatedTechRec.techRecord[1].statusCode).toEqual(STATUS.CURRENT);
        expect(updatedTechRec.techRecord[1]).not.toHaveProperty(
          "lastUpdatedAt"
        );
        expect(updatedTechRec.techRecord[1]).not.toHaveProperty(
          "lastUpdatedByName"
        ...
        expect(updatedTechRec.techRecord[0].lastUpdatedById).toEqual(
          createdById
        );
        expect(updatedTechRec.techRecord[0].lastUpdatedByName).toEqual(
          createdByName
        );
        expect(updatedTechRec.techRecord[0].statusCode).toEqual(
          STATUS.ARCHIVED
        );
        expect(updatedTechRec.techRecord[0].updateType).toEqual(
          UPDATE_TYPE.TECH_RECORD_UPDATE
        );
      });
    });
    ...
    context("and the vehicle has only a provisional tech record", () => {
      it("should create new CURRENT, set the audit details and archive the previous provisional record", async () => {
        ...
        const mockDAO = new MockDAO();
        const techRecordStatusHandler =
          new TechRecordStatusHandler<PublicServiceVehicle>(
            new TechRecordsListHandler<PublicServiceVehicle>(mockDAO)
          );

        const updatedTechRec: PublicServiceVehicle =
          await techRecordStatusHandler.prepareTechRecordForStatusUpdate(
            systemNumber,
            undefined,
            createdById,
    ...
    context("and the vehicle has a current and a provisional record", () => {
      it("should create new CURRENT, set the audit details and archive the previous current and provisional record", async () => {
        ...
        const updatedTechRec: HeavyGoodsVehicle =
          await techRecordStatusHandler.prepareTechRecordForStatusUpdate(
            systemNumber,
            undefined,
            createdById,
        ...
        expect(updatedTechRec.techRecord[2].createdByName).toEqual(
          createdByName
        );
        expect(updatedTechRec.techRecord[2].statusCode).toEqual(STATUS.CURRENT);
        expect(updatedTechRec.techRecord[2]).not.toHaveProperty(
          "lastUpdatedAt"
        );
        expect(updatedTechRec.techRecord[2]).not.toHaveProperty(
          "lastUpdatedByName"
        ...
        expect(updatedTechRec.techRecord[1].lastUpdatedById).toEqual(
          createdById
        );
        expect(updatedTechRec.techRecord[1].lastUpdatedByName).toEqual(
          createdByName
        );
        expect(updatedTechRec.techRecord[1].updateType).toEqual(
          UPDATE_TYPE.TECH_RECORD_UPDATE
        );
        expect(updatedTechRec.techRecord[1].statusCode).toEqual(
          STATUS.ARCHIVED
        );
      });
    });
