...
defineFeature(feature, (test) => {
  ...
  test("AC1. GET request: All attributes applicable to HGVs are returned", ({
    given,
    when,
    then,
  }) => {
    ...
    then(
      "I return all the attributes applicable to HGV, from the linked excel",
      () => {
        expect(response.status).toEqual(200);
        expect(response.body[0].techRecord[0].statusCode).toEqual(
          "provisional"
        );
  ...
  test("POST request: HGV vehicle is created, and the appropriate attributes are automatically set", ({
    given,
    when,
    then,
    and,
  ...
  test("PUT request: HGV vehicle is updated, and the appropriate attributes are automatically set", ({
    given,
    when,
    then,
    and,
    ...
    then(
      "my PUT action adheres to the HGV validations, present in the linked excel, columns D-E",
      () => {
    ...
    and(
      "a new identical tech record is created, with the same status, and the updated attributes on it",
      () => {
        if (doNotSkipAssertionWhenAdrFlagIsDisabled) {
    ...
    and(
      "I am only able to update attributes within the techRecord[] array",
      () => {
        if (doNotSkipAssertionWhenAdrFlagIsDisabled) {
