...
export abstract class VehicleProcessor<T extends Vehicle> {
  ...
  protected updateVehicleIdentifiers(existingVehicle: T, updatedVehicle: T): T {
    ...
    updatedVehicle.techRecord[0].reasonForCreation = `VRM updated from ${previousPrimaryVrm} to ${primaryVrm}. ` + updatedVehicle.techRecord[0].reasonForCreation;
  ...
  protected async validateVrmWithHistory(newVehicle: T, existingVehicle: T) {
    ...
    if (primaryVrm && existingVehicle.primaryVrm !== primaryVrm) {
      ...
      primaryVrmRecords.map((wrapper) => wrapper.techRecord).forEach((techRecord) => allTechRecords.push(...techRecord));
   ...
   */
  public async updateVehicle(msUserDetails: IMsUserDetails, oldStatusCode?: enums.STATUS) {
    try {
      const data = await this.createAndArchiveTechRecord(this.vehicle, msUserDetails, oldStatusCode);
      const updatedData = await this.techRecordDAO.updateSingle(data);
      return this.techRecordsListHandler.formatTechRecordItemForResponse(updatedData.Attributes as T);
    } catch (error) {
      console.error(error);
  ...
  public async addNewProvisionalRecord(msUserDetails: IMsUserDetails): Promise<T> {
    ...
    try {
      ...
      const vehiclesFromDB = await this.techRecordsListHandler.getFormattedTechRecordsList(this.vehicle.systemNumber, enums.STATUS.ALL, enums.SEARCHCRITERIA.SYSTEM_NUMBER);
      if (vehiclesFromDB.length !== 1) {
        throw this.Error(500, enums.ERRORS.NO_UNIQUE_RECORD);
      }
      const uniqueRecord = vehiclesFromDB[0];

      if (uniqueRecord.techRecord.filter((techRecord) => techRecord.statusCode === enums.STATUS.PROVISIONAL).length) {
        throw this.Error(400, enums.ERRORS.CURRENT_OR_PROVISIONAL_RECORD_FOUND);
      }

      this.auditHandler.setCreatedAuditDetails(this.vehicle.techRecord[0], msUserDetails.msUser, msUserDetails.msOid, new Date().toISOString());
      ...
      return this.techRecordsListHandler.formatTechRecordItemForResponse(uniqueRecord);
    } catch (error) {
      console.error(error);
  ...
  public async updateTechRecordStatusCode(uniqueRecord: T) {
    ...
    return this.techRecordsListHandler.formatTechRecordItemForResponse(updatedTechRecord.Attributes as T);
  }

  public async archiveTechRecordStatus(systemNumber: string, techRecordToUpdate: T, userDetails: IMsUserDetails, reasonForArchiving: string) {
    const allTechRecordWrapper = await this.techRecordsListHandler.getTechRecordList(systemNumber, enums.STATUS.ALL, enums.SEARCHCRITERIA.SYSTEM_NUMBER);
    ...
    const techRecordToArchive = VehicleProcessor.getTechRecordToArchive(techRecordWithAllStatues, techRecordToUpdate.techRecord[0].statusCode);
    ...
    let updatedTechRecord;
    try {
      updatedTechRecord = await this.techRecordDAO.updateSingle(techRecordWithAllStatues);
    } catch (error) {
      console.error(error);
  ...
  public async updateEuVehicleCategory(systemNumber: string, newEuVehicleCategory: enums.EU_VEHICLE_CATEGORY, msUserDetails: IMsUserDetails) {
    const techRecordWrapper = (await this.techRecordsListHandler.getTechRecordList(systemNumber, enums.STATUS.ALL, enums.SEARCHCRITERIA.SYSTEM_NUMBER))[0];
    const nonArchivedTechRecord = techRecordWrapper.techRecord.filter((techRecord) => techRecord.statusCode !== enums.STATUS.ARCHIVED);
    if (nonArchivedTechRecord.length > 1) {
      throw this.Error(400, enums.HTTPRESPONSE.EU_VEHICLE_CATEGORY_MORE_THAN_ONE_TECH_RECORD);
    }
    if (nonArchivedTechRecord.length === 0) {
      throw this.Error(400, enums.ERRORS.CANNOT_UPDATE_ARCHIVED_RECORD);
    }
    if (nonArchivedTechRecord[0].euVehicleCategory) {
      return new HTTPResponse(200, enums.HTTPRESPONSE.NO_EU_VEHICLE_CATEGORY_UPDATE_REQUIRED);
    }
    ...
    let updatedTechRecord;
    try {
      updatedTechRecord = await this.techRecordDAO.updateSingle(techRecordWrapper);
    } catch (error) {
      throw this.Error(500, enums.HTTPRESPONSE.INTERNAL_SERVER_ERROR);
    }
    return new HTTPResponse(200, this.techRecordsListHandler.formatTechRecordItemForResponse(updatedTechRecord.Attributes as T));
  }
  /* #endregion */
  ...
  private async validateAndMapTechRecord(msUserDetails: IMsUserDetails) {
    ...
    this.auditHandler.setAuditDetailsAndStatusCodeForNewRecord(this.vehicle.techRecord[0], msUserDetails);
  ...
  private async createAndArchiveTechRecord(updatedVehicle: T, msUserDetails: IMsUserDetails, oldStatusCode: enums.STATUS | undefined): Promise<T> {
    ...
    try {
      ...
      const techRecToArchive = VehicleProcessor.getTechRecordToArchive(techRecordWithAllStatuses, oldStatusCode ? oldStatusCode : statusCode);

      // if status code has changed from provisional to current
      this.updateCurrentStatusCode(oldStatusCode, statusCode, techRecordWithAllStatuses, msUserDetails);

      updatedVehicle = this.updateVehicleIdentifiers(techRecordWithAllStatuses, updatedVehicle);
      ...
      mergeWith(newRecord, updatedVehicle.techRecord[0], VehicleProcessor.arrayCustomizer);
      if (oldStatusCode) {
  ...
  private updateCurrentStatusCode(oldStatusCode: enums.STATUS | undefined, statusCode: string, techRecordWithAllStatuses: T, msUserDetails: IMsUserDetails) {
    if (oldStatusCode && oldStatusCode === enums.STATUS.PROVISIONAL && statusCode === enums.STATUS.CURRENT) {
      const currentTechRec = techRecordWithAllStatuses.techRecord.find((techRec) => techRec.statusCode === enums.STATUS.CURRENT);
      // check if vehicle already has a current
      if (currentTechRec) {
        currentTechRec.statusCode = enums.STATUS.ARCHIVED;
        const date = new Date().toISOString();
        this.auditHandler.setLastUpdatedAuditDetails(currentTechRec, msUserDetails.msUser, msUserDetails.msOid, date);
      }
    }
  }

  private async getTechRecordList(systemNumber: string) {
    const data = await this.techRecordsListHandler.getTechRecordList(systemNumber, enums.STATUS.ALL, enums.SEARCHCRITERIA.SYSTEM_NUMBER);
    if (data.length !== 1) {
  ...
  private static getTechRecordToArchive(techRecord: Vehicle, statusCode: string): TechRecord {
    const recordsToArchive = techRecord.techRecord.filter((record) => record.statusCode === statusCode);

    if (recordsToArchive.length > 1) {
      throw handlers.ErrorHandler.Error(500, `Vehicle has more than one tech-record with status ${statusCode}`);
    }

    if (recordsToArchive.length === 0) {
      throw handlers.ErrorHandler.Error(404, `Vehicle has no tech-records with status ${statusCode}`);
    }
  ...
  private static checkStatusCodeValidity(statusCode: string, oldStatusCode?: string) {
    ...
    if (oldStatusCode === enums.STATUS.CURRENT && statusCode === enums.STATUS.PROVISIONAL) {
